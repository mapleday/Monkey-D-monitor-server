    ...
    protected void decode(final ChannelHandlerContext ctx, HttpObject msg, List<Object> out) throws Exception {
        ...
        if (msg instanceof HttpMessage) {
            ...
            if (is100ContinueExpected(m)) {
                ctx.writeAndFlush(CONTINUE).addListener(new com.sohu.sns.monitor.netty.ChannelFutureListener() {
                    @Override
            ...
            //文件如果超过maxContentLength时，不会在将请求流的数据往后解析
...
////                sb.append(StatusUtils.putStatus(303092).toString());
////                NettyUtil.writeResponse((HttpRequest)msg, sb, ctx.channel());
